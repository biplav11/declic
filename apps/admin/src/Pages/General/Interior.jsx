...
import { changeInteriorLoading, getInteriors } from "src/Redux/general";
...
export default function Interior() {
  ...
  const { interiorLoading: dataLoading, interior } = useSelector((state) => state.general);

  function getData() {
    dispatch(changeInteriorLoading(true));
  ...
  const data = interior.map((r) => ({ key: r.id, ...r }));
  ...
  return (
    <MainLayout selected="Interior" expanded="General" title="Interior" clickfunction={handleAdd}>
      <Wrapper loading={dataLoading} empty={interior.length === 0}>
        <Table onRow={rowEvent} bordered columns={columns} dataSource={data} onChange={scrollTop} />
        <AddUser {...{ addOpen, setAddOpen, api, getData }} />
        <EditUser {...{ editOpen, setEditOpen, id, setId, interior, api, getData }} />
      </Wrapper>
    </MainLayout>
  );
}
...
function AddUser({ addOpen, setAddOpen, api, getData }) {
  ...
  async function handleSubmit(values) {
    let [success, error] = notification(api, "create", "interior");
    try {
      await pb.collection("interior").create(values);
      success();
      setAddOpen(false);
      form.resetFields();
      getData();
  ...
  return (
    <Drawer footer={footerAdd} title="Add New Interior" placement="right" onClose={closeDrawer} open={addOpen}>
...
function EditUser({ editOpen, setEditOpen, id, interior, setId, api, getData }) {
  const [editForm] = Form.useForm();
  const [editRecord, setEditRecord] = useState({ country_code: "+221" });

  async function deleteConfirm() {
    let [delSuccess, delError] = notification(api, "delete", "interior");
    try {
      await pb.collection("interior").delete(id);
      setEditOpen(false);
      setId(null);
      delSuccess();
      getData();
  ...
  async function handleSubmit(val) {
    let [editSuccess, editError] = notification(api, "update", "interior");
    try {
      await pb.collection("interior").update(editRecord?.id, val);
      editSuccess();
      setEditOpen(false);
      editForm.resetFields();
      getData();
  ...
  useEffect(() => {
    let res = interior[interior.findIndex((e) => e.id === id)];
    editForm.setFieldsValue(res);
    setEditRecord(res);
  }, [id]);

  const footer = (
    <Popconfirm title="Delete User" description="Are you sure to delete this interior?" onConfirm={deleteConfirm} okText="Yes" cancelText="No">
