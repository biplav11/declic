...
import { changeSafetyLoading, getSafety } from "src/Redux/general";
...
export default function Safety() {
  ...
  const { safetyLoading: dataLoading, safety } = useSelector((state) => state.general);

  function getData() {
    dispatch(changeSafetyLoading(true));
    dispatch(getSafety());
  }
  ...
  const data = safety.map((r) => ({ key: r.id, ...r }));
  ...
  return (
    <MainLayout selected="Safety" expanded="General" title="Safety" clickfunction={handleAdd}>
      <Wrapper loading={dataLoading} empty={safety.length === 0}>
        <Table onRow={rowEvent} bordered columns={columns} dataSource={data} onChange={scrollTop} />
        <AddUser {...{ addOpen, setAddOpen, api, getData }} />
        <EditUser {...{ editOpen, setEditOpen, id, setId, safety, api, getData }} />
      </Wrapper>
    </MainLayout>
  );
}
...
function AddUser({ addOpen, setAddOpen, api, getData }) {
  ...
  async function handleSubmit(values) {
    let [success, error] = notification(api, "create", "safety");
    try {
      await pb.collection("safety").create(values);
      success();
      setAddOpen(false);
      form.resetFields();
      getData();
  ...
  return (
    <Drawer footer={footerAdd} title="Add New Safety" placement="right" onClose={closeDrawer} open={addOpen}>
...
function EditUser({ editOpen, setEditOpen, id, safety, setId, api, getData }) {
  const [editForm] = Form.useForm();
  const [editRecord, setEditRecord] = useState({ country_code: "+221" });

  async function deleteConfirm() {
    let [delSuccess, delError] = notification(api, "delete", "safety");
    try {
      await pb.collection("safety").delete(id);
      setEditOpen(false);
      setId(null);
      delSuccess();
      getData();
  ...
  async function handleSubmit(val) {
    let [editSuccess, editError] = notification(api, "update", "safety");
    try {
      await pb.collection("safety").update(editRecord?.id, val);
      editSuccess();
      setEditOpen(false);
      editForm.resetFields();
      getData();
  ...
  useEffect(() => {
    let res = safety[safety.findIndex((e) => e.id === id)];
    editForm.setFieldsValue(res);
    setEditRecord(res);
  }, [id]);

  const footer = (
    <Popconfirm title="Delete User" description="Are you sure to delete this safety?" onConfirm={deleteConfirm} okText="Yes" cancelText="No">
