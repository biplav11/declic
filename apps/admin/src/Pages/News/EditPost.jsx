...
import { getNewsCategories, getSinglePost } from "src/Redux/news";
...
export default function Categories() {
  ...
  const [success, error] = notification(api, "update", "News Post");
  ...
  async function handleSubmit(val) {
    try {
      ...
      await pb.collection("news").update(post.id, data);
      if (deletedImage.length > 0) {
        await pb.collection("news").update(id, {
          "gallery-": deletedImage,
        });
      }
      setTimeout(() => {
        setLoading(false);
  ...
  const props = {
    ...
    onRemove: (file) => {
      setDeletedImage([...deletedImage, file.name.split("/").pop()]);
    },
  ...
  return (
    <MainLayout selected="Posts" expanded="News" title="Edit Post" clickfunction={null}>
