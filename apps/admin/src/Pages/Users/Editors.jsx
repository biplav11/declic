...
import { changeLoading, getEditors } from "src/Redux/users";
...
export default function Editors() {
  ...
  const { editorsLoading: dataLoading, editors } = useSelector((state) => state.user);

  const [delSuccess, delError] = notification(api, "delete", "editor");
  ...
  async function deleteConfirm(id) {
    try {
      ...
      showPbError(api, "Delete editor failed", err);
      delError();
    }
  }
    ...
    },
    {
      ...
      render: (_, record) => (
        <Flex gap={6}>
          ...
          <Popconfirm {...getPopMessage("Editor")} onConfirm={() => deleteConfirm(record.id)} okText="Yes" cancelText="No">
  ...
  const data = editors.map((r) => ({ key: r.id, ...r }));
  ...
  return (
    <MainLayout selected="Editors" expanded="Users" title="Editors" clickfunction={openAdd}>
      <Wrapper loading={dataLoading} empty={editors.length === 0}>
...
function EditorDrawer({ open, record, onClose, onSaved, api }) {
  const [form] = Form.useForm();
  const [saving, setSaving] = useState(false);
  const isEdit = !!record;
  const [success] = notification(api, isEdit ? "update" : "create", "editor");
  ...
  async function handleSubmit(values) {
    ...
    try {
      ...
      // Only send auth-managed flags when their value actually differs from
      // what's stored. PB rejects writes to these fields unless the caller
      // matches the collection's manageRule; staying silent on unchanged
      // values avoids a needless rejection when creating via an editor.
      ...
      showPbError(api, isEdit ? "Update editor failed" : "Create editor failed", err);
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Drawer
      forceRender
      title={isEdit ? `Edit ${record?.email || "Editor"}` : "Add New Editor"}
      placement="right"
      width={520}
      onClose={onClose}
      open={open}
      footer={footer}
      extra={isEdit ? <Tag color="blue">Editor</Tag> : null}
    >
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item label="Email" name="email" rules={[{ required: true }, { type: "email" }]}>
          <Input placeholder="eg. editor@declic.com" autoComplete="off" />
        </Form.Item>
