...
import { getBrands, getModels } from "src/Redux/vehicles";
...
export default function Variants() {
  ...
  const [success, error] = notification(api, "create", "Variants");
  ...
  async function handleSubmit(val) {
    try {
      ...
      const res = await pb.collection("variants").create(data);
      let slug = await checkSlug(convertToSlug(val.title.trim()), res.id);
      if (slug !== res.slug) {
        await pb.collection("variants").update(res.id, {
          slug: slug,
        });
      }
      setTimeout(() => {
