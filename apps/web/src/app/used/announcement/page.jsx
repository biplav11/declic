...
import { NewCarsCard, UsedCarsCard } from "@/components/Common";
...
export default function page() {
  ...
  const items = ["Keywords", "Price", "Body", "Number of Doors", "Energy", "Fiscal Power", "Transmission"].map(
    (item, i) => ({
      key: i,
      label: <span style={{ textTransform: "uppercase", fontSize: 11, color: iconColor }}>{item}</span>,
      children: i === 0 ? search : text,
    })
  );

  const usedCarsTitle = {
    title: "Announcement of the day",
  ...
  return (
    <Container>
      <Gap height={30} />
      <Title level={2}>{usedCarsTitle.title}</Title>
      <Row gutter={20}>
        <Col xs={0} lg={6}>
          <div style={{ border: "1px solid", borderColor: borderColor, borderRadius: 10 }}>
